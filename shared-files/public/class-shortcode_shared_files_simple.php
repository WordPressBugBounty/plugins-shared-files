...
class ShortcodeSharedFilesSimple {
    public static function view( $atts ) {
        ...
        $html .= '<div class="' . $elem_class . ' shared-files-simple-container" data-search-type="' . sanitize_title( SharedFilesHelpers::searchType() ) . '" data-post-id="' . intval( get_the_ID() ) . '" data-search-log-nonce="' . wp_create_nonce( 'shared_files_search_log' ) . '">';
        ...
        if ( isset( $_GET['sf_category'] ) && $_GET['sf_category'] != '0' ) {
            ...
            if ( isset( $_GET['c'] ) && $_GET['c'] != 'all_files' ) {
                ...
                if ( $pagination_active ) {
                    if ( !empty( $_GET['_page'] ) ) {
                        $paged = intval( wp_unslash( $_GET['_page'] ) );
