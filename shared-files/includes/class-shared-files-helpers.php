...
class SharedFilesHelpers {
    public static function createDir( $target_dir, $create_index_php = 0 ) {
        require_once ABSPATH . 'wp-admin/includes/file.php';
        $s = get_option( 'shared_files_settings' );
        WP_Filesystem();
        global $wp_filesystem;
        if ( !$wp_filesystem->is_dir( $target_dir ) ) {
            $result = $wp_filesystem->mkdir( $target_dir );
            if ( $result ) {
                SharedFilesHelpers::writeLog( 'Folder created successfully: ' . $target_dir );
                if ( $create_index_php ) {
                    $file = $target_dir . 'index.php';
                    $content = '<?php // Automatically generated by Shared Files ?>';
                    $success = $wp_filesystem->put_contents( $file, $content );
                }
            } else {
                SharedFilesHelpers::writeLog( 'Failed to create folder: ' . $target_dir );
            }
        }
    }
    ...
    public static function getExportUploadDir() {
        ...
        if ( !file_exists( $dir ) || !is_dir( $dir ) ) {
            SharedFilesHelpers::createDir( $dir, 1 );
            if ( is_dir( $dir ) ) {
    ...
    public static function getExportRandomFilename( $type ) {
        $rand = md5( wp_rand() );
        $filename = 'shared_files_' . $type . '_' . wp_date( 'Y-m-d' ) . '_' . substr( $rand, 0, 12 ) . '.csv';
        return $filename;
    }
    ...
    public static function getIPAddress() {
        foreach ( array(
            ...
            if ( array_key_exists( $key, $_SERVER ) === true ) {
                $server_key = sanitize_text_field( wp_unslash( $_SERVER[$key] ) );
                foreach ( explode( ',', $server_key ) as $ip ) {
                    $ip = trim( $ip );
    ...
    public static function getPreviewButton( $file_id, $file_url, $atts = [] ) {
        ...
        if ( in_array( $filetype, $image_types ) ) {
            ...
            if ( isset( $s['file_open_method'] ) && $s['file_open_method'] == 'redirect' ) {
                ...
                $file_url_parts = wp_parse_url( $file_url );
